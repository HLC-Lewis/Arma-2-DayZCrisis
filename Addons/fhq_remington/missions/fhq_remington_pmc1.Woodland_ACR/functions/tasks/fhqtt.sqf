 ...
 * 
 * Unit expressions:
 * Whenever a unit expression is asked for, there are several possibilities to 
...
{
    private ["_units", "_subject", "_topic", "_text", "_unitsArray"];
...
FHQ_TT_createSimpleTask =
{
    private ["_currentUnits", "_currentTask", "_currentTaskState", "_taskObjects"];
    _currentUnits = _this select 0;
    _currentTask = _this select 1;
...
{
    private ["_currentUnits", "_tasks", "_count", "_i", "_current", "_taskObjects", "_task", "_state"];
    _currentUnits = _this select 0;
...
FHQ_TT_addTasks =
{
    private ["_numEntries", "_unitPool", "_currentUnits", "_currentTaskList", "_current", "_i"];
...
FHQ_TT_hasTask =
{
    ...
    {
        if (((_x select 0) select 0) == _this) exitWith {_result = true;};
...
FHQ_TT_UpdateTaskList =
{
    ...
    if (_count > 0) then
    {
    	for [ {_i = 0}, {_i < _count}, {_i = _i + 1}] do                
        {
            ...
            _currentTask = _current select 1; // [name, text, title, waypoint, object/position]
            _currentUnits = _current select 0;
            _currentTaskState = _current select 2;
            
             if ((_currentTask select 0) call FHQ_TT_hasTask) then
             {
                 private ["_localTask", "_x"];
